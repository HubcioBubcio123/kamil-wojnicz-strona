...
- [ ] `index.html` work section — the Płetfale and Przygotowanie Fizyczne cards play the same footage as their page heroes on hover (`plywanie-hover.mp4`; `przygotowanie-fizyczne-hero.mp4#t=9.5`, which rests on the hero poster's frame instead of the clip's black first frame). The Fizjoterapia card still uses a picsum placeholder: replace it with a real photo, save as `.webp` in `assets/img/`
- [x] `pletfale.html` hero — now uses `assets/img/pletfale/plywanie-hover.mp4` as a full-bleed background video, matching `index.html`'s hero pattern. It has no poster: the `kamil-wojnicz-basen.webp` poster was removed per request because it flashed before the video started, so iPhones that refuse autoplay (Low Power Mode) show a plain hero with no fallback photo, which the user accepted.
...
- [x] `przygotowanie-fizyczne.html` process steps — 3 real photos of Kamil, cropped to 4:3 and saved as `.webp` in `assets/img/przygotowanie-fizyczne/`. Steps 1-2 come from `KamilGoogleDISC/` (HEIC originals copied to `assets/source/KamilPhotos/`); step 3 is a portrait the user sent directly (`assets/source/KamilPhotos/KamilBoxSit.jpg`), which replaced the squat-rack photo.
